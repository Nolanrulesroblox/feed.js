...
function processLike() {
  ...
  if (clicked_btn.getAttribute("like") === 'like' /*clicked_btn.classList.contains('fa-thumbs-o-up')*/) {
      action = 'like';
  } else if (clicked_btn.getAttribute("like") === 'unlike' /*clicked_btn.classList.contains('fa-thumbs-up')*/) {
      action = 'unlike';
  }else{
    return;
  }
  var request = new XMLHttpRequest();
  request.open('POST', '/api/v1/?k=like', true);
...
function f(lines=3,id,turnid) {
  const linesElement= lines;
  const truncateElement= document.querySelector(id);
  ...
  const getLineHeight = function(element) {
    ...
    while(lines * lineHeight < truncateElement.clientHeight) {
      ...
      truncateElement.innerHTML = truncateTextParts.join(' ') +'<div style="display:contents;cursor:pointer;" trunid="'+turnid+'">...</div>';
    //  turnid.removeEventListener('click',lineschange)
     // document.querySelector('[trunid="'+turnid+'"]').addEventListener('click',lineschange)
    }
  lineschange = function(){
    truncateElement.innerHTML = truncateText;
    const truncateTextParts= truncateText.split(' ');
    const lineHeight = getLineHeight(truncateElement);
    const lines = parseInt(99);
    ...
    while(lines * lineHeight < truncateElement.clientHeight) {
      ...
      truncateElement.innerHTML = truncateTextParts.join(' ') +'<div style="display:contents;cursor:pointer;" trunid="'+turnid+'">...</div>';
      turnid.removeEventListener('click',lineschange)
      turnid.addEventListener('click',lineschange)
    }
    }, 5000);
  }
}
